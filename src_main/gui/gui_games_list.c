...
const char *	get_games_statement_sql = QUOTE(select game.id, 
	game.		steam_appid, 
...
game.			name, 
	game.		allowed, 
	game.		enabled, 
	game.profile_id from game order by game.name;);

const char *	update_game_statement_sql = QUOTE(update game set steam_appid = ?, 
	name =		?, 
	allowed =	?, 
	enabled =	?, 
	profile_id = ? where id = ?);
...
static void deserialize_game(sqlite3_stmt * stmt, game_t * out_game)
{
	...
	const char *	name_temp = (const char *)
...
bool get_non_steam_game_path(char * szPath);

void gui_games_list_draw(gui_games_list_data_t * data)
{
	igBeginChild("GamesChild", (ImVec2) { 0, -28.0 * gui_common_get_data()->dpi_scale }, false, 
	...
	if (igButton("Add non-Steam game...", VEC2_ZERO))
	{
		char szPath[1024];
...
void gui_games_list_cleanup(gui_games_list_data_t * data)
{
	...
	db_owner_lock_and_get_db(data->db_owner);
...
void StartProcess( LPCWSTR args,char *szOut);
...
bool get_non_steam_game_path(char * szPath)
{
	...
	swprintf(cmd_line, MAX_PATH, L"%ls", exe_path);
	
	LOGI("cmd line: %ls\n", cmd_line);

	StartProcess(cmd_line,szPath);
	LOGI("GAME PATH:%s",szPath);
...
void StartProcess( LPCWSTR args,char *szOut)
{
	...
	if (CreatePipe(&hPipeRead, &hPipeWrite, &saOutPipe, 1024))
	{
		...
		if (CreateProcessW(NULL, (LPWSTR)args,
			...
			&processInfo))
		{
			if (WAIT_TIMEOUT != WaitForSingleObject(processInfo.hProcess, 30000))
			{
				...
				if (PeekNamedPipe(hPipeRead, NULL, 0, NULL, &dwReadLen, NULL) && dwReadLen > 0)
				{
					...
					if (ReadFile(hPipeRead, szOut, dwReadLen, &dwStdLen, NULL))
					{
						LOGI("%s",szOut);
					}
				}
