...
static void _handle_raw_input(LPARAM lParam) {
	...
	if (raw->header.dwType == RIM_TYPEKEYBOARD) {
		...
		static unsigned char dummyKeyState[256] = {0};
		static unsigned char keyState[256] = {0};
  ...
  //       (unsigned int)raw->data.keyboard.VKey);
		...
		if(1)
		{
			...
			if (desc) {
				...
				int bytes_written = ToUnicode(kb.VKey, kb.MakeCode, dummyKeyState, utf16_buffer, sizeof(utf16_buffer)/sizeof(utf16_buffer[0]), 0);
				if (bytes_written < 1) {
			...
			if ((kb.VKey == VK_CONTROL) || (kb.VKey == VK_SHIFT) || (kb.VKey == VK_MENU)) {
				...
				if (!is_down) {
					for (auto it = keys_down.begin(); it != keys_down.end();) {
						...
						bool should_delete =
							   ((kb.VKey == VK_CONTROL) && (it->ctrl))
							|| ((kb.VKey == VK_SHIFT) && (it->shift))
							|| ((kb.VKey == VK_MENU) && (it->alt));
			...
			key_fmt_buffer[0] = 0;
			if (any_ctrl)  wcscat(key_fmt_buffer, L"CTRL+");
			if (alt)   wcscat(key_fmt_buffer, L"ALT+");
			if (shift) wcscat(key_fmt_buffer, L"SHIFT+");
			...
			_codeduplicationftw_util_wide_to_utf8(key_fmt_buffer, wcslen(key_fmt_buffer), utf8_buffer, sizeof(utf8_buffer));
...
bool  gui_impl_init(const char* title) {
	...
	wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(NULL), NULL, NULL, NULL, NULL, title, NULL };
	...
	tbe = (NOTIFYICONDATAA*)malloc(sizeof(NOTIFYICONDATAA));
